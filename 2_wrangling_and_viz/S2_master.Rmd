...
The `is.na()` function returns "True" if something is `NA`, so `!is.na()` (read: "*not* is NA") returns the opposite.
...
Here, `shp` is a spatial dataframe -- you can manipulate it using the usual dplyr operations, but it has columns containing

Next, let's grab the relevant information from the listings data and turn it into an sf dataframe:
...
Finally, we plot the with leaflet.
```{r}
leaflet(by.zip) %>%
